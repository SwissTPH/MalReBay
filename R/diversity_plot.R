...
generate_allele_frequency_plot <- function(raw_data_df,
  ...
  plot_data_prepared <- raw_data_df %>%
    ...
    dplyr::mutate(
      marker_id = sub("_.*", "", allele_name),
      Day_Type = dplyr::if_else(grepl("Day 0", Sample.ID), "Day 0", "Day Failure"),
      Day_Type = factor(Day_Type, levels = c("Day 0", "Day Failure"))
    )
  
  # Calculate Frequencies
  frequency_data <- plot_data_prepared %>%
    dplyr::group_by(marker_id, Day_Type, raw_length) %>%
    dplyr::summarise(count = n(), .groups = 'drop') %>%
    dplyr::group_by(marker_id, Day_Type) %>%
    dplyr::mutate(frequency = count / sum(count)) %>%
    dplyr::ungroup()
  
  # Create the Plot
  allele_plot <- ggplot2::ggplot(data = frequency_data, ggplot2::aes(x = raw_length, y = frequency, fill = Day_Type)) +
