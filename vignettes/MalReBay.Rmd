...
![Overview of the Bayesian inference framework implemented in MalReBay.](figure/Bayesian_Framework2.png){width="100%"}
...
The main `MalReBay()` function automates this process—from loading the genotyping data and parameter settings to generating posterior estimates and final recurrence classifications. The following sections describe each input type and how to prepare your data for analysis.
...
![Length polymorphic](figure/angola_1.png){width="100%"}

### 4.2 Amplicon sequencing data

For amplicon sequencing, the marker columns store haplotype labels instead of allele lengths. The input Excel file has a similar structure to length-polymorphic data:

Here is an example of the expected input format for amplicon sequencing data:
![Amplicon sequencingt](figure/ampseq_1.png){width="100%"}
...
```{r MOI,fig.width=7, fig.height=5, message=FALSE, out.width='100%', fig.alt="Violin plot showing the multiplicity of infection per marker for the length-polymorphic dataset."}
...
##### Marker diversity
```{r marker-diversity-length-descriptive,fig.width=7, out.width='100%', fig.height=5, message=FALSE, fig.alt="A set of pie charts showing the allele diversity for each microsatellite marker."}
if (nrow(genotypedata_lp) > 0) {
...
# Define a quick config for the vignette
quick_config <- system.file("extdata", "default_mcmc_config.xlsx", package = "MalReBay")
...
The `MalReBay()` function returns a list containing two key data frames:
...
The `MalReBay()` function saves the MCMC log-likelihood data for each site within its results object. You can use the exported `plot_likelihood_diagnostics` function to view the plots for any site interactively in your R session.
...
To evaluate the performance of the Bayesian classification in MalReBay, we compare its results against a simpler **match-counting algorithm**. This algorithm counts allele matches between Day 0 and recurrence samples to provide a baseline for comparison.

For length-polymorphic markers, we combine the outputs of `MalReBay` and the `match-counting` algorithm into a single table. This allows us to examine, for each patient:
...
output_dir_nonconverge <- file.path(tempdir(), "malrebay_results", "non_convergence_test")
...
loglik_data <- results_nonconverge$mcmc_loglikelihoods[[site_to_plot]]
...
if (!is.null(loglik_data)) {
  ...
  cat("No log-likelihood data found for site: ", site_to_plot, "\n")
}
```
...
![MalReBay framework.](figure/MalReBay_Framework.png){width="100%"}
...
+ Diagnostic Reports: Functions such as `plot_likelihood_diagnostics()` produce trace plots and Gelman–Rubin plots for convergence checks.
