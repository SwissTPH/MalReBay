...
![MalReBay framework.](figure/Bayesian_Framework2.png){width=600px}
...
```{r convergence-diagnostics, echo=FALSE, fig.align='center'}
site_name <- "Benguela"
...
if (!is.null(LP_loglik_data)) {
  ...
  image_paths_LP <- file.path(output_dir_lp, "convergence_diagnosis", site_name,
    c("gelman_loglikelihood.png", "loglikelihood_traceplot.png",
      "loglikelihood_histogram.png", "loglikelihood_acf.png")
  )

  if (all(file.exists(image_paths_LP))) {
    knitr::kable(
      matrix(paste0("![](", image_paths_LP, "){width=100%}"), ncol = 2, byrow = TRUE),
      escape = FALSE
    )
  } else {
    knitr::kable("One or more diagnostic plots could not be found.")
  }
} else {
...
quick_mcmc_config <- list(
  ...
  ess_threshold = 400     # A relaxed effective sample size for the example
)
```
...
```{r convergence-diagnostics-ampseq, echo=FALSE, fig.align='center'}
site_name <- "1"
...
if (!is.null(ampseq_loglik_data)) {
  ...
  image_paths_ampseq <- file.path(output_dir_ampseq, "convergence_diagnosis", site_name,
    c("gelman_loglikelihood.png", "loglikelihood_traceplot.png",
      "loglikelihood_histogram.png", "loglikelihood_acf.png")
  )
  
  if (all(file.exists(image_paths_ampseq))) {
    knitr::kable(
      matrix(paste0("![](", image_paths_ampseq, "){width=100%}"), ncol = 2, byrow = TRUE),
      escape = FALSE
    )
  } else {
    knitr::kable("One or more diagnostic plots could not be found.")
...
main_output_dir <- here::here("vignettes", "malrebay_vignette_outputs")

nonconverge_subdir <- file.path(main_output_dir, "non_convergence_test")
if (!dir.exists(nonconverge_subdir)) {
  dir.create(nonconverge_subdir, recursive = TRUE)
}
...
classification_summary_nonconverge <- classify_infections(
  imported_data = imported_data,
  mcmc_config = quick_mcmc_config,
  output_folder = nonconverge_subdir,
  n_workers = 2,
  verbose = FALSE
)

site_to_check <- "Benguela"
loglik_data <- classification_summary_nonconverge$mcmc_loglikelihoods[[site_to_check]]

if (!is.null(loglik_data)) {
  generate_likelihood_diagnostics(
    all_chains_loglikelihood = loglik_data,
    site_name = site_to_check,
    save_plot = TRUE,
    output_folder = nonconverge_subdir, 
    verbose = FALSE
  )
  ...
  image_paths <- file.path(
    nonconverge_subdir, "convergence_diagnosis", site_to_check,
    c("gelman_loglikelihood.png", "loglikelihood_traceplot.png", 
      "loglikelihood_histogram.png", "loglikelihood_acf.png")
  )
  ...
  cat("No log-likelihood data found for site: ", site_to_check, "\n")
}
```
