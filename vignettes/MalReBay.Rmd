...
```{r MOI,fig.width=7, fig.height=5, message=FALSE}
...
##### Marker diversity
```{r marker-diversity-length-descriptive,fig.width=7, fig.height=5, message=FALSE}
...
if (!is.null(LP_loglik_data)) {
    ...
    site_name = site_name,
    save_plot = TRUE,
    output_folder = "vignettes/malrebay_vignette_outputs/length_polymorphic_results",
    verbose = FALSE
  )
image_paths_lp <- file.path(
    "vignettes/malrebay_vignette_outputs/length_polymorphic_results/convergence_diagnosis",
    site_name,
...
##### Multiplicity of Infection (MOI)  
```{r MOI_ampseq,fig.width=7, fig.height=5, message=FALSE}
...
##### Marker diversity
```{r marker-diversity-ampseq,fig.width=7, fig.height=5, message=FALSE}
...
if (!is.null(ampseq_loglik_data)) {
    ...
    site_name = site_name,
    save_plot = TRUE,
    output_folder = "vignettes/malrebay_vignette_outputs/ampseq_results",
    verbose = FALSE
  )
image_paths_amp <- file.path(
    "vignettes/malrebay_vignette_outputs/ampseq_results/convergence_diagnosis",
    site_name,
    c("gelman_loglikelihood.png",
      "loglikelihood_traceplot.png",
      "loglikelihood_histogram.png",
  ...
  if (all(file.exists(image_paths_amp))) {
    knitr::kable(
      matrix(paste0("![](", image_paths_amp, "){width=100%}"), ncol = 2, byrow = TRUE),
      escape = FALSE
    )
  } else {
    knitr::kable("Convergence diagnostic plots could not be found. Please check the file paths.")
...
classification_summary_nonconverge <- classify_infections(
  imported_data = imported_data,
  mcmc_config = quick_mcmc_config,
  output_folder = "vignettes/malrebay_vignette_outputs/non_convergence_test",
  n_workers = 2,
  verbose = FALSE
)
...
if (!is.null(loglik_data)) {
  ...
  image_paths <- file.path(
    "vignettes/malrebay_vignette_outputs/non_convergence_test/convergence_diagnosis", site_name,
